...
// —ложение двух матриц с использованием global пам€ти
template<class T> __global__ void __global__matrix_add__global__(T *a, T *b, T *c, int height, int width, size_t pitch1, size_t pitch2, size_t pitch3, MEMORY cache) {
...
// —ложение двух матриц с использованием constant пам€ти
template<class T> __global__ void __global__matrix_add__constant__(T *c, int height, int width, size_t pitch, MEMORY cache) {
	storage<T> buffer_a;
	storage<T> buffer_b;
	int sharedIndexX = (int)((height*(blockIdx.x+0))/gridDim.x);
	int sharedIndexY = (int)((width*(blockIdx.y+0))/gridDim.y);
...
// —ложение двух матриц с использованием texture пам€ти
template<class T> __global__ void __global__matrix_add__texture__(T *c, int height, int width, size_t pitch, MEMORY cache) {
...
template<class T> __host__ void __cdecl __host__matrix_add(dim3 blocks, dim3 threads, MATRIX<T> *a, MATRIX<T> *b, MATRIX<T> *c, MEMORY src, MEMORY dest, MEMORY cache) {
	...
	switch(dest) {
	case GLOBAL:
		err = cudaMallocPitch((void**)&d_c, &pitch3, c->width * sizeof(T), c->height);
		break;
	default:
		break;
	}
	...
	if (src == GLOBAL && dest == GLOBAL) {
		__global__matrix_add__global__<T><<<blocks,threads>>>(d_a, d_b, d_c, height, width, pitch1, pitch2, pitch3, cache);
	}
	else if (src == CONSTANT && dest == GLOBAL) {
		__global__matrix_add__constant__<T><<<blocks,threads>>>(d_c, height, width, pitch3, cache);
	}
	else if (src == TEXTURE && dest == GLOBAL) {
		__global__matrix_add__texture__<T><<<blocks,threads>>>(d_c, height, width, pitch3, cache);
	}
...
template __host__ void __cdecl __host__matrix_add<DATATYPE>(dim3 blocks, dim3 threads, MATRIX<DATATYPE> *a, MATRIX<DATATYPE> *b, MATRIX<DATATYPE> *c, MEMORY src, MEMORY dest, MEMORY cache);
