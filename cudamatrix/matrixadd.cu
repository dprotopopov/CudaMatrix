...
template<class T> __host__ void __host__matrix_add(dim3 blocks, dim3 threads, MATRIX<T> *a, MATRIX<T> *b, MATRIX<T> *c, MEMORY src, MEMORY dest, MEMORY cache) {
	...
	if (src == GLOBAL && dest == GLOBAL) {
		__global__matrix_add__global__<T><<<blocks,threads>>>(d_a, d_b, d_c, height, width, pitch1, pitch2, pitch3);
	}
	else if (src == CONSTANT && dest == GLOBAL) {
		__global__matrix_add__constant__<T><<<blocks,threads>>>(d_c, height, width, pitch3);
	}
	else if (src == TEXTURE && dest == GLOBAL) {
		__global__matrix_add__texture__<T><<<blocks,threads>>>(d_c, height, width, pitch3);
	}
